...
static dev_t scull_p_devno;			/* Our first device number */
...
#ifdef SCULL_DEBUG
static ssize_t scull_read_p_mem(struct file *filp, char __user *buf, size_t count,
                loff_t *f_pos)
{
	int i, len;
	struct scull_pipe *p;
	...
	len = sprintf(text, "Default buffersize is %i\n", scull_p_buffer);
	for(i = 0; i < scull_p_nr_devs && len <= limit; i++) {
		p = &scull_p_devices[i];
		if (mutex_lock_interruptible(&p->mutex))
			return -ERESTARTSYS;
		len += sprintf(text+len, "\nDevice %i: %p\n", i, p);
/*		len += sprintf(text+len, "   Queues: %p %p\n", p->inq, p->outq);*/
		len += sprintf(text+len, "   Buffer: %p to %p (%i bytes)\n", p->buffer, p->end, p->buffersize);
		len += sprintf(text+len, "   rp %p   wp %p\n", p->rp, p->wp);
		len += sprintf(text+len, "   readers %i   writers %i\n", p->nreaders, p->nwriters);
...
static struct file_operations scull_proc_p_ops = {
	.read = scull_read_p_mem,
	.owner = THIS_MODULE,
};
#endif
...
#ifdef SCULL_DEBUG
	proc_create("scullpipe", 0, NULL, &scull_proc_p_ops);
#endif
